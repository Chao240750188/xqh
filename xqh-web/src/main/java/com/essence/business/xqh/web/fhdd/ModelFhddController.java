...
@RestController
@RequestMapping("/modelFhdd")
public class ModelFhddController {

    @Autowired
    ModelFhddService modelFhddService;
    ...
    public SystemSecurityMessage getModelList() {
        try {
            Map<String,Object> results = modelFhddService.getModelList();
    ...
    public SystemSecurityMessage searchPlanIsExits(@PathVariable String planName) {
        try {
            Boolean isExits = modelFhddService.searchPlanIsExits(planName);
    ...
    public SystemSecurityMessage savePlanWithCache(@RequestBody ModelFhddPlanVo vo) {
        try {
            String results = modelFhddService.savePlan(vo);
            if ("planNameExist".equals(results)){
                return SystemSecurityMessage.getFailMsg("方案名称已存在，请重新输入！",null);
            }
            if (results == null){
                return SystemSecurityMessage.getFailMsg("防洪调度方案保存失败！",null);
            }
            return SystemSecurityMessage.getSuccessMsg("防洪调度方案保存成功",results);
    ...
    public SystemSecurityMessage getRainfallsInfo(@PathVariable String planId) {
        try {
            ...
            List<Map<String,Object>> results = modelFhddService.getRainfallsInfo(planinfo);
    ...
    public SystemSecurityMessage saveRainfallsFromCacheToDb(@PathVariable String planId) {
        try {
            ...
            modelFhddService.saveRainfallsFromCacheToDb(planinfo, results);
    ...
    public void exportRainfallTemplate(HttpServletRequest request, HttpServletResponse response, @PathVariable String planId) {
        try {
            ...
            Workbook workbook = modelFhddService.exportRainfallTemplate(planinfo);
            //响应尾
    ...
    public SystemSecurityMessage importRainfallData(@RequestParam(value = "files", required = true) MultipartFile mutilpartFile, @PathVariable String planId) {
        ...
        if (mutilpartFile == null) {
            ...
            // 解析表格数据为对象类型
            try {
                List<Map<String,Object>> list = modelFhddService.importRainfallData(mutilpartFile,planinfo);
    ...
    public SystemSecurityMessage savePlanInputZ(@RequestBody ModelFhddInputVo vo) {
        try {
            Boolean flag = modelFhddService.savePlanInputZ(vo);
            if (!flag){
                return SystemSecurityMessage.getFailMsg("防洪调度初始水位和下泄流量保存失败！",null);
            }
            return SystemSecurityMessage.getSuccessMsg("防洪调度初始水位和下泄流量保存成功",null);
    ...
    public SystemSecurityMessage modelCall(@PathVariable String planId) {
        ...
        modelFhddService.modelCall(planinfo);
    ...
    public SystemSecurityMessage getModelResultQ(@PathVariable String planId){
        ...
        Object results = modelFhddService.getModelResultQ(planInfo);
