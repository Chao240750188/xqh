...
import com.essence.business.xqh.dao.dao.fbc.FbcHdpHhtdzWDao;
...
@Service
public class ModelFbcGcdServiceImpl implements ModelFbcGcdService {
    ...
    public List<Object> getModelList() {
        ...
        List<YwkModel> modelList = ywkModelDao.getYwkModelByModelType("FBC_GCD");
    ...
    public List<Object> getModelRiverRoughness(String modelId) {
        ...
        for (YwkModelRoughnessParam roughnessParam : modelRoughnessList) {
            List<YwkRiverRoughnessParam> collect = paramMap.get(roughnessParam.getRoughnessParamid()).stream().sorted(Comparator.comparing(YwkRiverRoughnessParam::getMileage)).collect(Collectors.toList());
            roughnessParam.setParamList(collect);
     ...
     */
    @Override
    public Workbook exportBoundaryTemplate(String planId, String modelId) {
        //查询模型边界关联表
        ...
        for (YwkBoundaryBasic ywkBoundaryBasic : boundaryBasicList) {
            if (ywkBoundaryBasic.getBoundaryType().equals("1") || ywkBoundaryBasic.getBoundaryType().equals("3")) {
                llboundaryList.add(ywkBoundaryBasic);
            }
        }
        YwkPlaninfo planInfo = ywkPlaninfoDao.findOneById(planId);
        //封装边界模板数据
        ...
        for (int i = 0; i < llboundaryList.size(); i++) {
     ...
     */
    @Override
    public List<Object> getModelCwBoundaryBasic(ModelParamVo modelParamVo) {
        //先从缓存获取
        List<Object> dataCacheList = (List<Object>) CacheUtil.get("modelCwBoundary", modelParamVo.getnPlanid());
        if (dataCacheList != null)
            return dataCacheList;
        //如果缓存没有从数据库获取
        List<Object> list = new ArrayList<>();
     ...
     */
    @Override
    public Workbook exportCwBoundaryTemplate(String planId, String modelId) {
        //查询模型边界关联表
        List<YwkModelBoundaryBasicRl> modelBoundaryList = ywkModelBoundaryBasicRlDao.findByIdmodelId(modelId);
        //查询边界详细数据表
        List<String> stcdList = new ArrayList<>();
     ...
     */
    @Override
    public List<Object> importCwBoundaryData(MultipartFile mutilpartFile, String planId, String modelId) {
        List<Object> boundaryDataList = new ArrayList<>();
        //查询模型边界关联表
        List<YwkModelBoundaryBasicRl> modelBoundaryList = ywkModelBoundaryBasicRlDao.findByIdmodelId(modelId);
        //查询边界详细数据表
        ...
        //存入缓存
        CacheUtil.saveOrUpdate("modelCwBoundary", planId, boundaryDataList);
    ...
    public List<YwkPlanInfoBoundaryDto> savePlanCwBoundaryData(List<YwkPlanInfoBoundaryDto> ywkPlanInfoBoundaryDtoList, String planId) {
        ...
        if (ywkPlanInfoBoundaryDtoList != null && ywkPlanInfoBoundaryDtoList.size() > 0) {
            YwkPlanInfoBoundaryDto ywkPlanInfoBoundaryDto = ywkPlanInfoBoundaryDtoList.get(0);
            YwkBoundaryBasicDto boundary = ywkPlanInfoBoundaryDto.getBoundary();
            ywkPlaninFloodBoundaryDao.deleteByPlanIdAndStcd(planId, boundary.getStcd());
            //封装新数据
            YwkPlaninfo planinfo = ywkPlaninfoDao.findOneById(planId);
            //输出步长
            Date startTime = planinfo.getdCaculatestarttm();
            ...
            ywkPlaninFloodBoundaryDao.saveAll(planBoundaryList);
            //更新缓存
            CacheUtil.saveOrUpdate("modelCwBoundary", planId, ywkPlanInfoBoundaryDtoList);
    ...
    @Override
    public List<Object> getModelCwBoundaryByFbcPlan(ModelParamVo modelParamVo, String nPlanId) {
        List<Object> list = new ArrayList<>();
        YwkPlaninfo planInfo = ywkPlaninfoDao.findOneById(modelParamVo.getnPlanid());
        //封装时间列
        Date startTime = planInfo.getdCaculatestarttm();
        Date endTime = planInfo.getdCaculateendtm();
        //根据方案id获取风暴潮输出潮位数据
        List<FbcHdpHhtdzW> wList = fbcHdpHhtdzWDao.findByNPlanidOrderByAbsoluteTime(nPlanId);
        if(wList==null || wList.size()==0){
            return getModelCwBoundaryBasic(modelParamVo);
        }else{
            //存最后一条潮位数据
            FbcHdpHhtdzW fbcHdpHhtdzW = wList.get(wList.size() - 1);
            ...
            for (YwkBoundaryBasic ywkBoundaryBasic : boundaryBasicList) {
                ...
                for (Date time = startTime; time.before(DateUtil.getNextMinute(endTime, 1)); time = DateUtil.getNextHour(startTime, count)) {
                    ...
                    dataJsonObj.put("time", DateUtil.dateToStringNormal3(time));
                    try {
                        dataJsonObj.put("boundaryData", wList.get(count).getTdz());
                    }catch (Exception e){
                        dataJsonObj.put("boundaryData", fbcHdpHhtdzW.getTdz());
                    }
                    dataList.add(dataJsonObj);
                    count++;
                }
