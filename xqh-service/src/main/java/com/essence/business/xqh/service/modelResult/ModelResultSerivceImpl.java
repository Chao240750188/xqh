...
public class ModelResultSerivceImpl implements ModelResultService {
    ...
    public void getCsnlResultMaxDepthToPicture(List<GridResultDto> gridResultDtoList, String planId, Object processNum) {
        ...
        try {
            ...
            String fileName ="";
            if (null!=processNum && !StrUtil.isEmpty(processNum.toString())){//水深过程数据
                dbfFilePath = GisPathConfigurationUtil.getModelGridDbfFilesPath() + "/MODEL_NLFX/"+planId+ "/" + processNum;//生成shp后的存储路径
                fileName = "elements_depth_" + processNum;
            }else {//最大水深
                dbfFilePath = GisPathConfigurationUtil.getModelGridDbfFilesPath() + "/MODEL_NLFX/"+planId+ "/maxDepth";//生成shp后的存储路径
                fileName ="elements_maxDepth";
            }
            File filePathFile = new File(dbfFilePath);
            if (!filePathFile.exists()) {
            ...
            String exportPictureFormate = GisPathConfigurationUtil.getExportPictureFormate(); //导出图片格式：png
            //shp模板路径
            String shpTemplatePath = GisPathConfigurationUtil.getCsnlModelShpTempletePath();
            String maxDepthTemplate = GisPathConfigurationUtil.getMxdTemplateCSNL();//水深 mxd的文件名
