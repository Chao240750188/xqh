...
public class GisUtil {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {//如果状态码为200,就是正常返回
                ...
                if (null != responseEntity) {
                    ...
                    if (responseContent.contains("geometries")) {
                        ...
                        Gson gson = new Gson();
                        List<Object> geometriesList = gson.fromJson(gson.toJson(geometries), new TypeToken<List<Object>>() {
                        }.getType());
            ...
            if (response.getStatusLine().getStatusCode() == 200) {//如果状态码为200,就是正常返回
                ...
                if (null != responseEntity) {
                    ...
                    if (responseContent.contains("features")) {
                        ...
                        Gson gson = new Gson();
                        List<Object> featuresList = gson.fromJson(gson.toJson(features), new TypeToken<List<Object>>() {
                        }.getType());
    ...
    /**
     * 获取基础信息
     * searchText=%E9%A9%AC%E6%B2%B3
     * &contains=true&searchFields=STNM&sr=
     * &layers=5%2C6%2C7%2C9%2C8%2C10%2C1%2C2%2C3%2C4%2C11
     * &layerDefs=&returnGeometry=true&maxAllowableOffset=
     * &geometryPrecision=&dynamicLayers=&returnZ=false&returnM=false&gdbVersion=&f=pjson
     */
    public String getSpaceQuery(String gisUrl0, String searchText, String searchFields, String layers) throws Exception {
        // 创建默认的httpClient实例
        CloseableHttpClient httpclient = HttpClients.createDefault();
        // 创建httppost
        HttpPost httppost = new HttpPost(gisUrl0);
        ...
        params.add(new BasicNameValuePair("searchFields", searchFields));
        ...
        params.add(new BasicNameValuePair("dynamicLayers", ""));
        params.add(new BasicNameValuePair("returnZ", "false"));
        params.add(new BasicNameValuePair("returnM", "false"));
        ...
        String responseContent = EntityUtils.toString(responseEntity, "UTF-8");
        if (!responseContent.contains("results")) {
            throw new RuntimeException("调用GIS服务，错误信息：" + responseContent);
        }
        response.close();
        // 关闭连接,释放资源
        httpclient.close();
    ...
    public String getSpaceGeometry(String gisUrl0, String geometry, String layers, String sr) throws Exception {
        // 创建默认的httpClient实例
        CloseableHttpClient httpclient = HttpClients.createDefault();
        // 创建httppost
        HttpPost httppost = new HttpPost(gisUrl0);
...
//        params.add(new BasicNameValuePair("Geometry", geometry));
        ...
        params.add(new BasicNameValuePair("layers", layers));
        params.add(new BasicNameValuePair("layerDefs", ""));
        params.add(new BasicNameValuePair("time", layers));
        params.add(new BasicNameValuePair("layerTimeOptions", ""));
        params.add(new BasicNameValuePair("tolerance", "3"));
        params.add(new BasicNameValuePair("mapExtent", "114.81924957470812,34.423761639488134,122.12640004542585,38.14251998181174"));
        params.add(new BasicNameValuePair("imageDisplay", "856,866,96"));
        params.add(new BasicNameValuePair("returnGeometry", "true"));
        params.add(new BasicNameValuePair("maxAllowableOffset", ""));
        params.add(new BasicNameValuePair("returnZ", "false"));
        params.add(new BasicNameValuePair("returnM", "false"));
