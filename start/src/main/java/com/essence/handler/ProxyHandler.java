...
public class ProxyHandler {
		...
		try {
			try {
				...
				if (originalUri == null || originalUri.isEmpty()) {
					...
					sendErrorResponse(response, errorMessage, "400 - " + errorMessage,
				...
				if (originalUri.toLowerCase().startsWith("http%3a%2f%2f")
						|| originalUri.toLowerCase().startsWith("https%3a%2f%2f"))
					originalUri = URLDecoder.decode(originalUri, "UTF-8");

				// check the Referer in request header against the
				// allowedReferer in proxy.config
				...
				if (allowedReferers != null && allowedReferers.length > 0 && request.getHeader("referer") != null) {
					setReferer(request.getHeader("referer")); // replace
																// PROXY_REFERER
																// with real
																// proxy
					String httpReferer;
					try {
						...
						_log(Level.WARNING,
								"Proxy is being used from an invalid referer: " + request.getHeader("referer"));
						sendErrorResponse(response, "Error verifying referer. ", "403 - Forbidden: Access is denied.",
								HttpServletResponse.SC_FORBIDDEN);
						return;
					}
					if (!checkReferer(allowedReferers, httpReferer)) {
						_log(Level.WARNING,
								"Proxy is being used from an unknown referer: " + request.getHeader("referer"));
						sendErrorResponse(response, "Unsupported referer. ", "403 - Forbidden: Access is denied.",
								HttpServletResponse.SC_FORBIDDEN);
						return;
					}
				}

				// Check to see if allowed referer list is specified and reject
				// if referer is null
				if (request.getHeader("referer") == null && allowedReferers != null
						&& !allowedReferers[0].equals("*")) {
					_log(Level.WARNING, "Proxy is being called by a null referer.  Access denied.");
					sendErrorResponse(response,
							"Current proxy configuration settings do not allow requests which do not include a referer header.",
			...
			// Throttling: checking the rate limit coming from particular
			// referrer
			if (serverUrl.getRateLimit() > -1) {
				synchronized (_rateMapLock) {
					ConcurrentHashMap<String, RateMeter> ratemap = castRateMap(request.getAttribute("rateMap"));
					if (ratemap == null) {
					...
					if (!rate.click()) {
						_log(Level.WARNING, "Pair " + key + " is throttled to " + serverUrl.getRateLimit()
								+ " requests per " + serverUrl.getRateLimitPeriod() + " minute(s). Come back later.");
					...
					// making sure the rateMap gets periodically cleaned up so
			...
			// if token comes with client request, it takes precedence over
			// token or credentials stored in configuration
			boolean hasClientToken = requestUri.contains("?token=") || requestUri.contains("&token=")
					|| post.contains("?token=") || post.contains("&token=");
			String token = "";
			if (!hasClientToken) {
				// Get new token and append to the request.
				// But first, look up in the application scope, maybe it's
				// already there:
				...
				// if still no token, let's see if there are credentials stored
				// in configuration which we can use to obtain new token
				...
				if (token != null && !token.isEmpty() && !tokenIsInApplicationScope) {
					// storing the token in Application scope, to do not waste
					// time on requesting new one until it expires or the app is
					// restarted.
			...
			if (token == null || token.isEmpty() || hasClientToken) {
				// if token is not required or provided by the client, just
				// fetch the response as is:
				fetchAndPassBackToClient(con, response, true);
			} else {
				// credentials for secured service have come from configuration
				// file:
				// it means that the proxy is responsible for making sure they
				// were properly applied:

				// first attempt to send the request:
				boolean tokenRequired = fetchAndPassBackToClient(con, response, false);

				// checking if previously used token has expired and needs to be
				// renewed
				if (tokenRequired) {
					_log(Level.INFO, "Renewing token and trying again.");
					// server returned error - potential cause: token has
					// expired.
					// we'll do second attempt to call the server with renewed
					// token:
					token = getNewTokenIfCredentialsAreSpecified(serverUrl, requestUri);
	...
	private HttpURLConnection forwardToServer(HttpServletRequest request, String uri, byte[] postBody)
			throws IOException {
		...
		return postBody.length > 0 ? doHTTPRequest(uri, postBody, "POST", mapHeaderInfo)
				: doHTTPRequest(uri, request.getMethod());
	}

	// proxy gets the response back from server
	private boolean fetchAndPassBackToClient(HttpURLConnection con, HttpServletResponse clientResponse,
			boolean ignoreAuthenticationErrors) throws IOException {
		if (con != null) {
			...
			for (String headerFieldKey : headerFieldsSet) {
				...
				for (String value : headerFieldValue) {
					...
					// Note: this might not be what everyone expects, but it
					// helps some users
					...
					// StackOverflow
					// http://stackoverflow.com/questions/31452074/how-to-proxy-http-requests-in-spring-mvc
			...
			// if the content of the HttpURLConnection contains error message,
			// it means the token expired, so let proxy try again
			String strResponse = buffer.toString();
			if (!ignoreAuthenticationErrors && strResponse.contains("error")
	...
	private boolean passHeadersInfo(Map<String, String> mapHeaderInfo, HttpURLConnection con) {
		Iterator<Map.Entry<String, String>> headerIterator = mapHeaderInfo.entrySet().iterator();
	...
	// simplified interface of doHTTPRequest, will eventually call the complete
	// interface of doHTTPRequest
	...
	private HttpURLConnection doHTTPRequest(String uri, byte[] bytes, String method, Map<String, String> mapHeaderInfo)
			throws IOException {
		URL url = new URL(uri);
	...
	private String getNewTokenIfCredentialsAreSpecified(ServerUrl su, String url) throws IOException {
		...
		if (isUserLogin || isAppLogin) {
			...
			if (isAppLogin) {
				...
				// "App Login" - authenticating using client_id and
				// client_secret stored in config
				...
				String uri = su.getOAuth2Endpoint() + "token?client_id=" + URLEncoder.encode(su.getClientId(), "UTF-8")
						+ "&client_secret=" + URLEncoder.encode(su.getClientSecret(), "UTF-8")
				...
				// if a request is already being made to generate a token, just
				// let it go
				...
				if (!infoUrl.isEmpty()) {
					...
					if (tokenServiceUri == null || tokenServiceUri.isEmpty()) {
						...
						// If the tokenServiceUri does not exist, check the
						// owningSystemUrl for token endpoint
					...
					if (tokenServiceUri != null && !tokenServiceUri.isEmpty()) {
						...
						String uri = tokenServiceUri + "?f=json&request=getToken&referer="
								+ URLEncoder.encode(PROXY_REFERER, "UTF-8") + "&expiration=60&username="
								+ URLEncoder.encode(su.getUsername(), "UTF-8") + "&password="
								+ URLEncoder.encode(su.getPassword(), "UTF-8");
						String tokenResponse = webResponseToString(doHTTPRequest(uri, "POST"));
						token = extractToken(tokenResponse, "token");
					}
				}
	...
	private boolean pathMatched(String allowedRefererPath, String refererPath) {
		...
		// If the allowedRefererPath contain a ending star and match the
		// begining part of referer, it is proper start with.
	...
	private boolean checkReferer(String[] allowedReferers, String referer) throws MalformedURLException {
		if (allowedReferers != null && allowedReferers.length > 0) {
			...
			for (String allowedReferer : allowedReferers) {
				...
				// since the allowedReferer can be a malformedURL, we first
				// construct a valid one to be compared with referer
				// if allowedReferer starts with https:// or http://, then exact
				// match is required
				if (allowedReferer.startsWith("https://") || allowedReferer.startsWith("http://")) {
					...
					// if allowedReferer starts with "//" or no protocol, we use
					// the one from refererURL to prefix to allowedReferer.
			...
			return false;// no-match in allowedReferer, does not allow the
							// request
		}
	...
	private boolean checkWildcardSubdomain(String allowedRefererDomain, String refererDomain)
	...
	private String exchangePortalTokenForServerToken(String portalToken, ServerUrl su) throws IOException {
		...
		String uri = su.getOAuth2Endpoint().substring(0, su.getOAuth2Endpoint().toLowerCase().indexOf("/oauth2/"))
				+ "/generateToken?token=" + URLEncoder.encode(portalToken, "UTF-8") + "&serverURL="
				+ URLEncoder.encode(url, "UTF-8") + "&f=json";
	...
	private String getJsonValue(String text, String key) {
		...
		if (i > -1) {
			...
			value = (value.length() > 0 && value.charAt(0) == '"') ? value.substring(1, value.indexOf('"', 1))
					: value.substring(0, Math.max(0,
							Math.min(Math.min(value.indexOf(","), value.indexOf("]")), value.indexOf("}"))));
		}
					...
					if (logger.getUseParentHandlers()) {
						...
						if (logLevelStr != null) {
							try {
								...
								System.err.println(dt.format(new Date()) + ": " + logLevelStr
										+ " is not a valid logging level.  Defaulting to SEVERE.");
							}
						}
	...
	public static class ProxyConfig {
		public boolean canReadProxyConfig() {
			InputStream configFile = ProxyConfig.class.getResourceAsStream("/proxy.config");
			...
			if (configFile != null) {
				...
				if (found) {
					...
					if (proxyConfigAttributes != null && !proxyConfigAttributes.isEmpty()) {
						...
						String allowedReferers = ProxyConfig.getAttributeWithRegex("allowedReferers",
								proxyConfigAttributes);
						...
						p = Pattern.compile("<\\s*serverUrls\\s*>(.+?)<\\s*/\\s*serverUrls\\s*>",
						...
						if (found) {
							...
							if (serverUrls != null && !serverUrls.isEmpty()) {
								p = Pattern.compile("<\\s*serverUrl (.+?)((<\\s*/\\s*serverUrl\\s*)|/)>",
										Pattern.MULTILINE | Pattern.DOTALL);
								...
								while (m.find()) {
									...
									String rateLimitPeriod = ProxyConfig.getAttributeWithRegex("rateLimitPeriod",
											server);
									String tokenServiceUri = ProxyConfig.getAttributeWithRegex("tokenServiceUri",
											server);
									String hostRedirect = ProxyConfig.getAttributeWithRegex("hostRedirect", server);

									serverList.add(new ServerUrl(url, matchAll, oauth2Endpoint, username, password,
											clientId, clientSecret, rateLimit, rateLimitPeriod, tokenServiceUri,
											hostRedirect));
								}
		...
		public ServerUrl getConfigServerUrl(String uri) {
			...
			for (ServerUrl su : serverUrls) {
				// if a relative path is specified in the proxy configuration
				// file, append what's in the request itself
				...
				int i;
				// try to match configUrl to the requested url, including
				// protocol
				...
				if (i == configUriParts.length) {
					// if the urls don't match exactly, and the individual
					// matchAll tag is 'false', don't allow
			...
			if (this.mustMatch)
				return null;// if nothing match and mustMatch is true, return
							// null
			else
				return new ServerUrl(uri); // if mustMatch is false send the
											// server URL back that is the same
											// the uri to pass thru
		}
	...
	public static class ServerUrl {
		...
		public ServerUrl(String url, String matchAll, String oauth2Endpoint, String username, String password,
				String clientId, String clientSecret, String rateLimit, String rateLimitPeriod, String tokenServiceUri,
				String hostRedirect) {
		...
		public int getRateLimitPeriod() {
			return (this.rateLimitPeriod == null || this.rateLimitPeriod.isEmpty()) ? -1
					: Integer.parseInt(this.rateLimitPeriod);
		}
	...
	private static void sendErrorResponse(HttpServletResponse response, String errorDetails, String errorMessage,
                                          int errorCode) throws IOException {
		response.setHeader("Content-Type", "application/json");
		String message = "{" + "\"error\": {" + "\"code\": " + errorCode + "," + "\"details\": [" + "\"" + errorDetails
	...
	private static void _sendURLMismatchError(HttpServletResponse response, String attemptedUri) throws IOException {
		sendErrorResponse(response,
				"Proxy has not been set up for this URL. Make sure there is a serverUrl in the configuration file that matches: "
						+ attemptedUri,
	...
	private static void _sendPingMessage(HttpServletResponse response, String version, String config, String log)
		...
		response.setHeader("Content-Type", "application/json");
		String message = "{ " + "\"Proxy Version\": \"" + version + "\"" +
		// ", \"Java Version\": \"" + System.getProperty("java.version") + "\""
		// +
				", \"Configuration File\": \"" + config + "\"" + ", \"Log File\": \"" + log + "\"" + "}";
		OutputStream output = response.getOutputStream();
		output.write(message.getBytes());
		output.flush();
	}
	...
	private String uriHostRedirect(String originalUri, ServerUrl serverUrl) throws MalformedURLException {
		if (serverUrl.hostRedirect != null && !serverUrl.hostRedirect.isEmpty()) {
			...
			redirectHost = redirectHost.endsWith("/") ? redirectHost.substring(0, redirectHost.length() - 1)
					: redirectHost;
